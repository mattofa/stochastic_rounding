...
# Read first file
df1 = pd.read_csv("sr_error.csv", header=None).T  # Transpose to get single column
df1.columns = ["y_values"]
df1["y_values"] = df1["y_values"].apply(Decimal)
df1["x_values"] = df1.index * 1000  # Scale X by 1000

# Read second file
df2 = pd.read_csv("alt_sr_error.csv", header=None).T  # Transpose
